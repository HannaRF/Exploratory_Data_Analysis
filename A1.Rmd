...
<br>  
Vamos verificar as frequências de algumas variáveis categóricas:

```{r, warning = FALSE, message = FALSE}
paged_table(data.frame(Tabela %>% count(animal),Tabela %>% count(furniture), Tabela %>% count(city)))
```  
<br>  

### Agora a frequência conjunta {.tabset} 

####  Animais x Mobiliária

```{r, warning = FALSE, message = FALSE}
kable(prop.table(table(Tabela$animal,Tabela$furniture)))
```  
  
Geralmente casas por imobiliária barram pets.  

####  Animais x Cidade

```{r, warning = FALSE, message = FALSE}
kable(prop.table(table(Tabela$animal,Tabela$city)))
```  

####  Cidade x Mobiliária

```{r, warning = FALSE, message = FALSE}
kable(prop.table(table(Tabela$city,Tabela$furniture)))
```  
...
Vamos criar uma nova coluna "preço_ relativo_m2" obtida pela razão entre o total em valor e a área. Vejamos os que possuem o $m^2$ acima de R$200,00.  
...
Tabela %>% filter(preço_m2 >=150) %>% ggplot(aes(preço_m2))  + geom_point(stat = 'count')
```  
<br>  

vejamos se opreço por $m^2$ tem alguma relação com a cidade.  
...
banheiros <- Tabela %>% ggplot( aes(x = bathroom, fill = city ) ) +
  geom_bar() +
  labs(x = "número de banheiros",fill = "é na cidade") + 
  theme(axis.title.y = element_blank())
...
estruturas de dados categóricos.Usei `patchwork` para juntar gráficos distintos.E `ggExtra` para plotar gráficos marginais.
